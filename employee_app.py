...
SELECT
    ...
    CASE 
        WHEN t1.is_fired THEN 'увол.'
        WHEN t1.is_maternity_leave THEN 'декр.'
        ELSE 'работ.'
    END AS status3,
...
@employee_app_bp.route('/employees_list', methods=['GET'])
@login_required
def get_employees_list():
    ...
    if role not in (1, 4, 7):
        ...
        conn, cursor = login_app.conn_cursor_init_dict("users")
        ...
        if role not in (1, 4, 7):
            ...
            cursor.execute(
                ...
                SELECT
                    ...
                    t8.contractor_name,
                    t9.pers_num {order} 1 AS pers_num,
                    concat_ws(' ', t1.last_name, t1.first_name, t1.surname) AS name,
                    ...
                    COALESCE(t7.work_days, 0) {order} 0.01 AS work_days,
                    COALESCE(t10.hours, 0) {order} 0.01 AS hours,
                    t1.labor_status,
            ...
            if employee:
                ...
                col_13 = employee["work_days"]
                col_14 = employee["hours"]
                col_15 = ""
...
def save_employee():
    ...
    if role not in (1, 4, 7):
        ...
        employee_data['salary_sum'] = payment_app.convert_amount(employee_data['salary_sum'])
        ...
        if len(values_empl) > 1:
            ...
            action_empl = 'UPDATE'
            query_empl = payment_app.get_db_dml_query(action=action_empl, table='users', columns=columns_empl)
            print(query_empl)
            print(values_empl)
        ...
        if values_p_n[0]:
            ...
            query_p_n = payment_app.get_db_dml_query(action=action_p_n, table='personnel_number', columns=columns_p_n,
                                                     expr_set=expr_set)
            print(expr_set)
            print(query_p_n)
            print(values_p_n)
        ...
        if values_e_d[1]:
            ...
            query_e_d = payment_app.get_db_dml_query(action=action, table='empl_dept', columns=columns_e_d)
            print(query_e_d)
            print(values_e_d)
        ...
        if values_s[1]:
            ...
            query_s = payment_app.get_db_dml_query(action=action, table='salaries', columns=columns_s)
            print(query_s)
            print(values_s)
        ...
        if values_h_p_d_n[1]:
            ...
            query_h_p_d_n = payment_app.get_db_dml_query(action=action, table='hour_per_day_norm',
        ...
        if values_h_a_f[1] or values_h_a_f[2]:
            ...
            query_h_a_f = payment_app.get_db_dml_query(action=action, table='hire_and_fire', columns=columns_h_a_f)
            print(query_h_a_f)
            print(values_h_a_f)
...
def get_sort_filter_data(page_name, limit, col_1, col_1_val, col_id, col_id_val, filter_vals_list, user_id,
    ...
    where_expression = (
        ...
        f"({payment_app.conv_data_to_db(list_type_col[col_num], col_1_val, all_col_types)}, "
        f"{payment_app.conv_data_to_db(sort_col_id, col_id_val, all_col_types)})")
...
def get_header_menu(role: int = 0, link: str = '', cur_name: int = 0):
    ...
    if role == 1 or role == 4:
        header_menu = [
            {'link': f'/employees_list', 'name': 'Сотрудники'}
        ]
    elif role == 7:
        header_menu = [
            {'link': f'/employees_list', 'name': 'Сотрудники'}
